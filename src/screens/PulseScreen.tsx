...
export default function PulseScreen() {
  ...
  const loadPage = useCallback((page: number, append: boolean, mood: string, tab: 'foryou' | 'following') => {
    ...
    setTimeout(() => {
      ...
      const synced = newPosts.map(p => ({ ...p, isSaved: savedList.includes(p.id) }));
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      ...
      setPosts(fresh.map(p => ({ ...p, isSaved: saved.includes(p.id) })));
      setNewPostsCount(0);
      setRefreshing(false);
    }, 1200);
  };
  ...
  const handleLike = (postId: string) => {
    setPosts(prev => prev.map(p => {
      ...
      if (liked) { incrementStat('reactionsSent', 1); incrementStat('pulseScore', 2); }
  ...
  const updatePostCount = (postId: string, type: 'comments' | 'shares', delta: number) => {
    setPosts(prev => prev.map(p => p.id === postId ? { ...p, [type]: p[type] + delta } : p));
  };
    ...
    >
      ...
      {activeUserIndex !== null && (
        <SparkViewer groupedSparks={groupedSparks} initialUserIndex={activeUserIndex}
          ...
          onSparkViewed={(id: string) => setViewedSparks(prev => new Set([...prev, id]))}
