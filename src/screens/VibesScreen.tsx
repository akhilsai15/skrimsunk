...
function VibeCard({
  ...
  const [pulses, setPulses] = useState(vibe.pulseCount);
        ...
        {showComments && (
          ...
          >
            <div className="flex items-center justify-between px-5 mb-4">
              <span className="font-bold text-white">{fmt(vibe.comments)} Comments</span>
            ...
            <div className="px-4 mt-3 flex gap-2 items-center">
              ...
              <button className="w-10 h-10 rounded-full bg-[#B026FF] flex items-center justify-center">
...
export default function VibesScreen() {
  ...
  useEffect(() => {
    setLoading(true);
    setTimeout(() => {
      const initial = assembleVibesFeed(mood, 0, 12);
      setVibes(initial);
      setLoading(false);
    }, 600);
  }, [mood]);
  ...
  useEffect(() => {
    if (!loadingMore && vibes.length > 0 && currentIdx >= vibes.length - 3) {
      setLoadingMore(true);
      setTimeout(() => {
        const more = assembleVibesFeed(mood, vibes.length, 8);
