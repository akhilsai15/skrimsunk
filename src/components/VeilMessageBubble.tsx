...
export function VeilMessageBubble({ 
  ...
  const [displayText, setDisplayText] = useState(isCloakMode ? message.text.split('').map(c => c.trim() ? '█' : ' ').join('') : message.text);
  ...
  const startCloakSequence = () => {
    ...
    const scrambleInterval = setInterval(() => {
      ticks++;
      setDisplayText(message.text.split('').map(c => c.trim() ? MATRIX_CHARS[Math.floor(Math.random() * MATRIX_CHARS.length)] : ' ').join(''));
      if (ticks > 2) {
        clearInterval(scrambleInterval);
        setCloakState('cloaked');
        setDisplayText(message.text.split('').map(c => c.trim() ? '█' : ' ').join(''));
      }
    }, 50);
  };
  ...
  const startDecodeSequence = (overrideTimer?: number | null) => {
    ...
    const activeInterval = setInterval(() => {
      setDisplayText(message.text.split('').map((c, i) => {
        if (i < currentIdx) return message.text[i];
        if (!c.trim()) return ' ';
  ...
  const startRecloakSequence = () => {
    ...
    setCloakState('recloaking');
    let currentIdx = message.text.length - 1;
